...
&Тест
Процедура ТестДолжен_НеЗапрашиватьВерсиюОграниченнойСверхуЗависимости() Экспорт

	// Подготовка
	Определитель = _Поделка.НайтиЖелудь("ОпределительВерсийЗависимостей");
	Зависимости = Новый ЗависимостиПакета().Добавить("registry-error", "1.0.0", "2.0.0");
	...
	Ожидаем.Что(Результат[0].ИмяПакета).Равно("registry-error");
	Ожидаем.Что(Результат[0].МинимальнаяВерсия).Равно("1.0.0");
	Ожидаем.Что(Результат[0].МаксимальнаяВерсия).Равно("2.0.0");

КонецПроцедуры

&Тест
Процедура ТестДолжен_НеВключитьОграниченнуюСверхуЗависимостьВУстаревшие() Экспорт
	...
	// Действие
	Результат = Определитель.ТолькоУстаревшие().НайтиВерсии(Зависимости).СкопироватьВТаблицу();
