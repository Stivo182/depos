...
Процедура ТестДолжен_ПроверитьБазовыйСлучайИзФайла() Экспорт
	...
	// Действие
	Результат = Определитель.УстановитьФайлМанифеста(ПомощникТестирования.ФайлМанифеста).ПолучитьЗависимости();
...
Процедура ТестДолжен_ПроверитьБазовыйСлучайИзТаблицы() Экспорт
	...
	// Действие
	Результат = Определитель.УстановитьТаблицуЗависимостей(Зависимости).ПолучитьЗависимости();
...
Процедура ТестДолжен_ПроверитьФильтрПоИмениПакета() Экспорт
	...
	Для Каждого ТестовыйСлучай Из ТестовыеСлучаи Цикл
		...
		// Действие
		Результат = Определитель.ФильтроватьПоИмени(Шаблон).ПолучитьЗависимости();
...
Процедура ТестДолжен_ПроверитьТипВерсии() Экспорт
	...
	Для Каждого ТестовыйСлучай Из ТестовыеСлучаи Цикл
		...
		// Действие
		Результат = Определитель.УстановитьТипВерсии(ТипВерсии).ПолучитьЗависимости();
...
&Тест
Процедура ТестДолжен_ПроверитьВыбрасываниеИсключенияНеНайденФайлМанифеста() Экспорт
		
	// Подготовка
	Определитель = Поделка.НайтиЖелудь("ОпределительВерсийЗависимостей");
	Определитель.УстановитьФайлМанифеста("./tests/fixtures/unknown");
	
	// Действие и проверка
	Ожидаем.Что(Определитель)
		.Метод("ПолучитьЗависимости")
		.ВыбрасываетИсключение("Не найден файл манифеста './tests/fixtures/unknown'");

КонецПроцедуры

&Тест
Процедура ТестДолжен_ПроверитьВыбрасываниеИсключенияВозниклаОшибкаПриПолученииЗависимостей() Экспорт
		
	// Подготовка
	Определитель = Поделка.НайтиЖелудь("ОпределительВерсийЗависимостей");
	Определитель.УстановитьФайлМанифеста("./tests/fixtures/packagedef-syntax-error");
...
Функция ПолучитьОпределительВерсийЗависимостей()
	Определитель = Поделка.НайтиЖелудь("ОпределительВерсийЗависимостей");
	Определитель.УстановитьФайлМанифеста(ПомощникТестирования.ФайлМанифеста);
