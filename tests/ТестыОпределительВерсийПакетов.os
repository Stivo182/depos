...
&Тест
Процедура ТестДолжен_ПропуститьНенайденныйПакетИПродолжитьОбработку() Экспорт

	// Подготовка
	Определитель = _Поделка.НайтиЖелудь("ОпределительВерсийПакетов");

	// Действие
	ВерсииПакетов = Определитель.НайтиВерсии("missing, autumn");

	// Проверка
	Ожидаем.Что(ВерсииПакетов.Получить("missing")).Равно(Неопределено);
	Ожидаем.Что(ВерсииПакетов.Получить("autumn")).Равно("4.3.11");
