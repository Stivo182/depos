...
Процедура Обновить(ФайлМанифеста, Зависимости) Экспорт
	...
	Если Не ДоступноОбновлениеМанифеста(_ИмяФайлаМанифеста) Тогда
		ВызватьИсключение СтрШаблон("Не поддерживается обновление файла манифеста '%1'", _ИмяФайлаМанифеста);
	КонецЕсли;
...
Процедура ОбновитьФайлМанифеста()
	...
	Попытка
		_ХранилищеМанифеста.Записать(
			_ИмяФайлаМанифеста,
			НовоеСодержимое,
			СтрШаблон("Возникла ошибка при записи файла манифеста '%1'", _ИмяФайлаМанифеста)
		);
...
Функция ПрочитатьСодержимоеФайлаМанифеста()
	...
	Возврат _ХранилищеМанифеста.Прочитать(
		_ИмяФайлаМанифеста,
		СтрШаблон("Возникла ошибка при чтении файла манифеста '%1'", _ИмяФайлаМанифеста)
	);

КонецФункции

Функция ПолучитьСовпаденияВТекстеМанифеста(ТекстМанифеста)
	Возврат _РегулярноеВыражениеЗависимостей.НайтиСовпадения(ТекстМанифеста);
...
Процедура ПроверитьЗависимостиФайлаМанифеста()
	...
	Исключение
		_Лог.Отладка(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ТекстСообщения = "возникла ошибка при получении зависимостей из файла манифеста: " 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СтрШаблон(ТекстОшибкиНеудачнаяПроверка, ТекстСообщения);
	КонецПопытки;
