...
//
// Параметры:
//   Пакеты - Строка - Список имен пакетов через запятую, опционально с минимальными версиями в формате "имя@версия"
//                     (например, "autumn@4.3.3, asserts").
...
Функция ОпределитьЦелевуюВерсию(ИмяПакета, Знач МинимальнаяВерсия)
	...
	Исключение
		ВызватьИсключение СтрШаблон(
			"Ошибка получения версий для пакета '%1': %2",
			ИмяПакета, 
