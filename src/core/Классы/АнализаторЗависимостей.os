...
// Возвращаемое значение:
//   ЭтотОбъект - Для поддержки цепочки вызовов
Функция УстановитьКаталогиИсходников(Каталоги) Экспорт
...
// Устанавливает каталоги со вспомогательными исходниками для анализа использования пакетов.
//
...
// Возвращаемое значение:
//   ЭтотОбъект - Для поддержки цепочки вызовов
Функция УстановитьКаталогиДляРазработки(Каталоги) Экспорт
...
Функция ПроверитьЗависимости() Экспорт
	...
	ДобавитьИспользуемыеПакеты(
		_СканерИсходников.НайтиПакеты(ПолучитьКаталогиОтносительноМанифеста(_КаталогиВспомогательныхИсходников)),
		 Истина
	);
...
Процедура ЗагрузитьЗависимости()
