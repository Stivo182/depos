...
Процедура ПодготовитьРегулярноеВыражениеИмениПакета(Фильтр)
	...
	Исключение
		ВызватьИсключение Новый ПостроительОшибки(
			КодыОшибокDepos.РегулярноеВыражениеФильтраНекорректно(),
			СтрШаблон("Некорректное регулярное выражение '%1'", Фильтр)
		)
			.Причина(ИнформацияОбОшибке())
			.Подсказка("Исправьте синтаксис регулярного выражения.")
			.Построить();
	КонецПопытки;
