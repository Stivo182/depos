...
//
// Исключения:
//   - source.directory_not_found - Если один из каталогов исходников не существует
Функция НайтиПакеты(Каталоги) Экспорт
	...
	Если ТипЗнч(Каталоги) = Тип("Массив") Тогда
		...
		КонецЦикла;
	Иначе
		КаталогиДляСканирования = СтрРазделить(Каталоги, ",", Ложь);
	КонецЕсли;
	...
	Для Каждого Каталог Из КаталогиДляСканирования Цикл
		Если Не ФС.КаталогСуществует(Каталог) Тогда
			ВызватьИсключение ФабрикаОшибок.Создать(
				КодыОшибокDepos.КаталогИсходниковНеНайден(),
				СтрШаблон("Каталог исходников '%1' не найден", Каталог),
