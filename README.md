...
Если пул не указан, для OpenHub используется пул по умолчанию. Если он указан у последнего проверяемого сервера,
после неудачных попыток depos проверяет на этом сервере пул по умолчанию. Уже проверенный пул того же реестра повторно
не запрашивается.
