...
Для работы требуется OneScript 2.1.0 или новее.
...
depos check [-m|--manifest <путь>] [-p|--package <имя>] [-d|--deprecated]
            ...
            [--registry <URL>] [--registry-proxy <URL>]
```
...
| --registry <URL> | Адрес реестра пакетов. |
...
Для `check` и `upgrade` адрес реестра и proxy можно задать опциями командной строки или переменными среды.
Адреса указываются полностью, вместе со схемой.

| Переменная среды | Опция командной строки | Значение по умолчанию |
| --- | --- | --- |
| `DEPOS_REGISTRY_URL` | `--registry` | `https://hub.oscript.io` |
| `DEPOS_REGISTRY_PROXY` | `--registry-proxy` | Proxy не используется |

Опция командной строки имеет приоритет над переменной среды.

Для `https://hub.oscript.io` тип API определяется автоматически. Пока адрес обслуживает старый хаб, `depos`
читает HTML-страницы пакетов. После переезда адреса на OpenHub `depos` автоматически использует основной публичный
пул и JSON API.
