...
| --registry <адрес|имя> | Адрес реестра или имя сервера из `opm.cfg`. |
...
Для команд `check` и `upgrade` depos использует серверы пакетов и proxy из `opm.cfg`. Настройки ищутся в том же
порядке, что и в OPM: в текущем каталоге, профиле пользователя, системном каталоге и рядом со стартовым сценарием.
Используется первый найденный файл.

Переменная `OPM_HUB_MIRROR` позволяет выбрать сервер из `opm.cfg` по имени. Без неё серверы проверяются в порядке
их объявления.
